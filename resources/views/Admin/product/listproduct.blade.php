...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    if (!catId) {
        e.preventDefault();
        alert('Please select a category');
        document.getElementById('edit_cat_id').focus();
        return;
    }
    ...
    if (!brandId) {
        e.preventDefault();
        alert('Please select a brand');
        document.getElementById('edit_brand_id').focus();
        return;
    }

    // Dynamically set the form action with the product ID
    if (productId) {
        this.action = "{{ route('products.product_update', ['id' => ':id']) }}".replace(':id', productId);
    } else {
        e.preventDefault();
