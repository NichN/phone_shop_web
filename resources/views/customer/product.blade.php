        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center mb-3">
                <div class="dropdown me-3">
                    <button class="btn dropdown-toggle" style="background-color: #2e3b56; color:white;" type="button"
                        data-bs-toggle="dropdown">
                        Find by
                    </button>
                    <ul class="dropdown-menu p-3">
                        <li class="mb-4"><input type="radio" name="category" class="form-check-input me-2" checked>All
                            Item</li>
                        @foreach ($brands as $brand)
                            <li class="mb-4">
                                <input type="radio" name="category" class="form-check-input me-2"
                                    value="{{ $brand->id }}" id="brand_{{ $brand->id }}">
                                <label for="brand_{{ $brand->id }}">{{ $brand->name }}</label>
                            </li>
                        @endforeach
                        <li class="mb-4"><button class="btn mt-2 w-100"
                                style="background-color: #2e3b56; color:white;">Apply</button></li>
                    </ul>
                </div>
            </div>
                    ...
                    @else
                        <div class="col-12">
                            <div class="d-flex align-items-center justify-content-center"
                                style="height:400px;">
                                <p class="fs-4 text-muted m-0">No product found</p>
                            </div>
                        </div>
                    @endif
                </div>
