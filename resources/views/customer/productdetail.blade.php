...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (matchingVariants.length > 0) {
        ...
        stockDisplay.textContent = variant.stock > 0 ? variant.stock : 'Out of stock';
        stockDisplay.className = variant.stock > 0 ? 'fw-bold text-primary' : 'fw-bold text-danger';

        addCartBtn.dataset.productItemId = variant.id;
        addCartBtn.dataset.price = variant.price;
        addCartBtn.disabled = variant.stock <= 0;

        if (matchingVariants.length > 1) {
            const typeList = matchingVariants.map(v => v.type).join(' / ');
            typeDisplay.innerHTML = `${typeList}<span class="text-danger">*</span>`;
        }
    } else {
        // If no matching variant found
    ...
    increaseQtyBtn?.addEventListener('click', () => {
        ...
        if (current < 999) quantityInput.value = current + 1;
    });
